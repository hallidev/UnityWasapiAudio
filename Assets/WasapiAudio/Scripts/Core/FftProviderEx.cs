...
namespace Assets.WasapiAudio.Scripts.Core
{
    ...
    public class FftProviderEx
    {
        ...
        public virtual void Add(float left, float right)
        {
            lock (_lockObject)
            {
                _storedSamples[_currentSampleOffset].Y = 0f;
                _storedSamples[_currentSampleOffset].X = (left + right) / 2f;
        ...
        public virtual void Add(float[] samples, int count)
        {
            ...
            lock (_lockObject)
            {
                ...
                {
                    _storedSamples[_currentSampleOffset].Y = 0f;
                    _storedSamples[_currentSampleOffset].X = MergeSamples(samples, i, _channels);
                    _currentSampleOffset++;
        ...
        public virtual bool GetFftData(Complex[] fftResultBuffer)
        {
            ...
            FastFourierTransform.FFT(true, _fftSizeExponent, input);
        ...
        public virtual bool GetFftData(float[] fftResultBuffer)
        {
            ...
            for (int i = 0; i < input.Length; i++)
            {
                fftResultBuffer[i] = input[i].Value();
            }
